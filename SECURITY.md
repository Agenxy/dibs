...
| Admin role | Granted only on the human's admin path. A role declared in `[roles]` names a string, and a name is free to take, so the declaration alone grants nothing: `[roles.identity]` must carry that agent's fingerprint, and without one the grant is refused forever rather than handed to whoever registers first. **Withdrawing it is the config**: a role this mechanism granted, and can prove it granted through `roles.pinned`, is demoted when the config stops authorising that fingerprint. A role a person granted by hand has no pin and is not touched; `dibs admin member <agent>` is the tool for that one. This row used to describe the superseded model, where the first registrant was pinned; that let an agent which had read `dibs.toml`, or guessed a likely line, win a two-minute race for the god view |
| Coordinator role | The admin path, and two others, both deliberate. What the role can reach is bounded by audit rather than by proof in one place: a coordinator may move an abandoned mailbox onto a third party and not onto itself, and "third party" is decided from evidence the transport stamped (where the target was registered from, which sessions it holds). A coordinator that speaks the raw API with the local secret can register a puppet with an invented session and move a mailbox onto it, and no rule the daemon can apply tells that puppet from a stranger, because nothing about a session is proven. Every adoption is ledgered under the coordinator's name with the target's provenance, so it is visible after the fact, and the role is granted by a person on that understanding. A human may approve a `request` carrying `grant: "coordinator"`, which is the human deciding through the ordinary response path rather than a bypass of it. And on a board with NO coordinator, an agent that can read `coordinator.claim` in the data directory may take the role once: that is how a fleet with nobody at the keyboard gets one at all. The claim is single-use, is refused once any coordinator exists, and is not minted when the config has actually DECIDED who coordinates, which means a declared name with a matching `[roles.identity]` fingerprint. A bare name decides nothing, because it can never be granted, and suppressing the claim on one left a board with no coordinator and no way to get one. This row used to say "only on the human's admin path", which was false in a security document, and the direction of the error was the wrong one |
| Lineage (`parent`) | Grants nothing unless the parent vouched with a one-time nonce |
| Host bridge (`dibs://wake` stream, `POST /api/wake-result`, `GET /api/hosts`) | Opened with the board's secret, the same credential as every other call. A bridge states which machine it is and which harnesses its own `[wake.exec]` table can start; the hub hands it wakes for agents on that machine and takes its report as the exit status. What it receives is the fixed wake sentence, the agent id, the sender's id, the mail type and the harness thread id: never a body. A bridge can state a host it is not on, which is exactly as strong as the shared secret already makes any bridge (docs/NETWORK.md §2, §6): what that buys is receiving that host's wake requests and reporting on them, not reading anyone's mail or running anything on the hub. The hub never runs a command for a remote agent and never learns the remote argv |
| Agent-supplied index (`POST /api/index`) | Accepted only with the token of an agent registered INSIDE the tree the payload names, and only for a tree the daemon has itself found unreadable; an agent whose repository the daemon resolved may name that repository and not a parent of it. The index scores the agents inside that tree and nothing else: an agent whose repository root is another repository is never scored by it, even beneath the shipped root, and it is never a peer of another project, because its identity fields are the shipper's word. What it can shape is the suggestions of agents in the same unreadable tree, which is the tree the shipper already has read access to. Scores declarations and nothing else: it decides no claim, no role, no membership, and its scores are suggestions only, whatever `join_threshold` and `auto_join` say for trees the daemon indexed itself (they used to pass through, so an agent's own index data could join other agents into a space; found by the pre-release review). File paths and commit subjects, never contents, bounded in count and bytes |
