...
| Agent-supplied index (`POST /api/index`) | Accepted only with the token of an agent registered INSIDE the tree the payload names, so an agent can supply the index for its own tree and no other. Used only for a tree the daemon could not read itself; the daemon's own reading wins wherever it has one. Scores declarations and nothing else: it decides no claim, no role, no membership. File paths and commit subjects, never contents, bounded in count and bytes |
