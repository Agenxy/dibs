 ...
 *
 * Install: copy to ~/.pi/agent/extensions/lanes.ts (global) or
 * .pi/extensions/lanes.ts (project-local). Both are auto-discovered and can be
...
const DIR = process.env["LANES_DIR"] ?? `${homedir()}/.lanes`
 ...
 *
 * hook_poll names the lane whether or not it has news, precisely so a caller
 ...
 * reattach a fresh agent onto a dead agent's lane and its mail. Mirrors the
...
export default function (pi: ExtensionAPI) {
  ...
  async function registerLanesTools(notify?: (m: string, level: string) => void) {
      ...
      pi.registerTool({
        ...
        async execute(_toolCallId: string, params: unknown, _signal: unknown, _onUpdate: unknown, ctx: any) {
          ...
          if (t.name === "register_lane") {
            ...
            // sibling lane and the original's mail becomes unreachable.
    ...
    registered = true
    notify?.(`Lanes: ${tools.length} coordination tools available`, "info")
  }
   ...
   *
   * This is the pi equivalent of the opencode `chat.message` hook: Lanes stays a
   ...
   */
  /**
   * Stamp a spawned subagent with the lane that spawned it.
   *
  ...
  pi.on("tool_call", async (event: any, ctx: any) => {
    try {
      ...
      if (typeof cmd !== "string" || cmd.includes("LANES_PARENT=")) return
      if (!stampable(cmd)) return
      const lane = await laneOf(sessionIdOf(ctx))
      if (lane) event.input.command = `LANES_PARENT=${lane} ${cmd}`
    } catch {
