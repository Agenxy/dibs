...
Feb 2026) is an MCP client, so Lanes needs no adapter.
...
mcp_servers:
  lanes:
    command: /Users/you/.local/bin/lanes
    args:
      - mcp-stdio
    enabled: true
    env:
      LANES_HARNESS: hermes
```

`lanes mcp-stdio` reads the daemon secret from disk, so no credential belongs in
this file.
...
### `LANES_HARNESS` is not optional here
...
in this survey has. Lanes does not need it today (loopback + local secret), but
it is the natural path if Lanes is ever reached across a network boundary that
...
- `hermes mcp add` connects and discovers all **25** Lanes tools.
- A real agent registered a lane and acknowledged the board unprompted.
- Identity lands correctly: the stdio bridge supplies `host`, `cwd` and `branch`
  for free, and `LANES_HARNESS` supplies the name the SDK does not.
