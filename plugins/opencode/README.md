...
Mail reaches an opencode agent without Lanes driving anything. Confirmed, not
inferred.
...
```
SchemaError: Expected a string starting with "prt", got "lanes-1785053881727-i5blt1"
...
After reading the mail, the model tried `get_message(191)` and got
...
> how does it reclaim its lane?

**This was decided and built.** It is recorded here because the reasoning is
worth keeping, not because it is still open.

A nonce is not a property of persistent lanes: `register_lane` accepts one from
any lane, of any kind, and re-registering with the same name and nonce reattaches
to the existing lane with its mail and claims rather than creating a sibling.
`resume_lane(nonce)` remains the path for a persistent lane reactivating after
sleeping.

A registration that presents a `session_id` already bound to a live lane also
...
- opencode's `initialize` negotiates `2025-11-25` and it never sends
  `resources/list`, so Lanes resources are pull-only here (same as Codex).
