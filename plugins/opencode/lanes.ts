 ...
 *
 * Install: copy to ~/.config/opencode/plugin/lanes.ts (global) or
 * .opencode/plugin/lanes.ts (project-local). opencode scans
 * {plugin,plugins}/*.{ts,js}.
 *
 * Env: LANES_ADDR (default 127.0.0.1:4777), LANES_DIR (default ~/.lanes)
 */
import type { Plugin } from "@opencode-ai/plugin"

const ADDR = process.env["LANES_ADDR"] ?? "127.0.0.1:4777"
const DIR =
  process.env["LANES_DIR"] ?? `${process.env["HOME"] ?? "."}/.lanes`

/**
 * The name Lanes knows this session by.
 *
 ...
 * the daemon could not match it to a lane, fell open the way it is designed to,
 * and the agent overwrote a file another lane held exclusively.
 *
 ...
 * process, and they share a bridge, a lane and therefore an id. That is the
...
async function guard(sessionID: string, path: string): Promise<string | null> {
  ...
  const res = await fetch(`http://${ADDR}/mcp`, {
    method: "POST",
    headers: { "content-type": "application/json", "X-Lanes-Local": key },
    body: JSON.stringify({
      jsonrpc: "2.0", id: 1, method: "tools/call",
      params: { name: "guard_path", arguments: { session_id: sessionID, path, cwd: process.cwd() } },
    }),
    // This sits in front of every edit. If Lanes is slow the edit proceeds.
...
/**
 * Ask Lanes what this session has waiting. Read-only: hook_poll never consumes
...
async function poll(sessionID: string): Promise<string | null> {
  ...
  const res = await fetch(`http://${ADDR}/mcp`, {
    ...
    // The user is waiting on their turn: never hang it on Lanes being slow.
    signal: AbortSignal.timeout(1500),
  })
  if (!res.ok) return null
 ...
 */
/**
 * A monotonic count of turns this process has taken, reported to Lanes.
 *
...
async function lane(): Promise<string | null> {
  const key = await secret()
  if (!key) return null
  try {
    const res = await fetch(`http://${ADDR}/mcp`, {
      ...
      // In front of every shell command the agent runs: never hang one on Lanes.
      signal: AbortSignal.timeout(1500),
    })
    if (!res.ok) return null
    const body = (await res.json()) as { result?: { content?: Array<{ text?: string }> } }
...
export const LanesPlugin: Plugin = async () => {
  return {
     ...
     */
    /**
     * Stamp every shell command with the lane that issued it, so a subagent it
     * spawns can be attributed to that lane when it stalls.
     *
    ...
    "shell.env": async (_input: unknown, output: { env: Record<string, string> }) => {
      ...
      // ancestor instead.
      if (output.env["LANES_PARENT"]) return
      const id = await lane()
      if (id) output.env["LANES_PARENT"] = id
    },
