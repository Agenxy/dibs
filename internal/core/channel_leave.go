...
// Leaving a lane, and the repair every departure implies.
//
// A departure is never just a deletion: the leaver may hold the lane
...
func (s *State) applyLaneLeave(l *Lane, op *Op, now time.Time) (Result, []Event, error) {
	ch := s.Channels[cleanID(op.Channel)]
	if ch == nil {
		return nil, nil, errf("E_NO_LANE", "nothing to leave", "no lane %s", op.Channel)
	}
	if _, ok := ch.Members[l.ID]; !ok {
		...
		if i := slices.Index(ch.Queue, l.ID); i >= 0 {
			...
			evs := []Event{{Type: "lane.left", Lane: l.ID, Data: map[string]any{
...
func (s *State) departChannel(ch *Channel, agent string) []Event {
	...
	//
	// Scoped to THIS channel: an agent leaving one lane still owes what it owes
	// everywhere else.
	var evs []Event
	evs = append(evs, s.dropAckRequirementsIn(ch.ID, agent)...)
	evs = append(evs, Event{Type: "lane.left", Lane: agent, Data: map[string]any{
	...
	ch.Owner = ""
	evs = append(evs, Event{Type: "lane.released", Lane: agent, Data: map[string]any{
		"lane_id": ch.ID,
	...
	// marked an agent stale, the owner left, and the lane's owner became the
	...
	for i, next := range ch.Queue {
		...
		ch.Owner = next
		return append(evs, Event{Type: "lane.joined", Lane: next, Data: map[string]any{
			"lane_id": ch.ID, "from_queue": true, "members": len(ch.Members),
		}}, Event{Type: "lane.exclusive", Lane: next, Data: map[string]any{
			"lane_id": ch.ID, "owner": next,
		}})
	}
	// Nobody waiting can take it. Leaving the lane locked-open with a queue that
