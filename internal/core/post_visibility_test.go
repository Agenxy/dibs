...
func TestAPostGoesToTheLaneAndNotTheBoard(t *testing.T) {
	...
	if _, _, err := st.Apply(&Op{
		Kind: OpLaneOpen, Token: author, Channel: "work", Text: "the topic",
	}, now); err != nil {
		t.Fatal(err)
	}
	if _, _, err := st.Apply(&Op{
		Kind: OpLaneJoin, Token: member, Channel: "work", Score: 0.9, ScorerID: "t",
	}, now); err != nil {
		t.Fatal(err)
	}
	if _, _, err := st.Apply(&Op{Kind: OpLaneSubscribe, Token: watcher, Channel: "work"}, now); err != nil {
		t.Fatal(err)
	}

	const secret = "THE-BODY-OF-THE-POST"
	_, evs, err := st.Apply(&Op{Kind: OpLanePost, Token: author, Channel: "work", Body: secret}, now)
	if err != nil {
		t.Fatal(err)
	}
	...
	if strings.Contains(string(blob), secret) {
		t.Errorf("the post body is in the event payload, which reaches every "+
			"authenticated lane: %s", blob)
	}
	if len(evs) != 1 || evs[0].Type != "lane.post" {
	...
	for _, who := range []struct {
		...
		if err != nil {
			t.Errorf("%s cannot even reach the lane: %v", who.name, err)
			continue
		}
		if !ok {
	...
	if _, err := st.ReaderChannel(st.LaneByToken(outsider), "work"); err == nil {
		t.Error("an outsider was allowed to read a lane it neither joined nor subscribed to")
	}

	// The subscriber must not receive the coordination key: it is held by
	// membership, and it is the one identity claim Lanes can verify.
	if ch.Key == "" {
		t.Fatal("precondition: the lane should have a key")
	}
}
...
func TestPostHistoryIsBounded(t *testing.T) {
	...
	if _, _, err := st.Apply(&Op{Kind: OpLaneOpen, Token: tok, Channel: "w", Text: "t"}, now); err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 40; i++ {
		if _, _, err := st.Apply(&Op{
			Kind: OpLanePost, Token: tok, Channel: "w", Body: "post " + itoa(i),
		}, now); err != nil {
			t.Fatal(err)
		}
	}
...
func TestAMergeCarriesThePostsAcross(t *testing.T) {
	...
	for tok, lane := range map[string]string{boss: "src", other: "dst"} {
		if _, _, err := st.Apply(&Op{Kind: OpLaneOpen, Token: tok, Channel: lane, Text: "t"}, now); err != nil {
			t.Fatal(err)
		}
		if _, _, err := st.Apply(&Op{
			Kind: OpLanePost, Token: tok, Channel: lane, Body: "said in " + lane,
		}, now); err != nil {
			t.Fatal(err)
		}
	}
	...
	if _, _, err := st.Apply(&Op{
		Kind: OpLaneMerge, Token: boss, Channel: "src", To: "dst",
	}, now); err != nil {
		t.Fatalf("merge: %v", err)
	}
	if st.Channels["src"] != nil {
		t.Fatal("precondition: the source lane should be gone after a merge")
	}
	var bodies []string
	for _, p := range st.Channels["dst"].Posts {
