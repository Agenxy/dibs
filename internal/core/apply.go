...
func (s *State) applyRespond(l *Agent, op *Op, now time.Time) (Result, []Event, error) {
	...
	asker := s.Agents[m.From]
	if asker != nil && asker.Status == StatusArchived {
		// Idle, not gone: the row and its mailbox are kept so it can resume
		// to exactly this. Said so, rather than "closed", which is the
		// story the responder was told about an agent that had merely gone
		// quiet (pre-release review, round five).
		res["note"] = "recorded; " + m.From + " is archived (idle for a long time, not " +
			"closed) and reads this if it resumes, which its credential still allows"
	}
	if asker.Retired() {
		res["delivered"] = false
