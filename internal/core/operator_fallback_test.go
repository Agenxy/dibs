...
//
// An agent that finishes work can find its recipient gone: agents get reaped,
// and the agent that asked for the work may be the one that ended. A reviewer hit
// this exactly: its report was addressed to a reaped agent, the refusal listed
// live agents, and it concluded there was no durable delivery path. It then tried
// broadcast, which is coordinator-only and correctly refused, and the review
// survived only in its own stdout.
//
// The path existed and nothing pointed at it. The operator's agent is persistent,
...
func TestAMissingRecipientPointsAtTheOperator(t *testing.T) {
	...
	if serr == nil {
		t.Fatal("sending to an agent that does not exist succeeded")
	}
	...
	if !strings.Contains(hint, "operator") {
		t.Errorf("the refusal does not name the operator's agent, so an agent with a "+
			"report and a dead recipient still has nowhere to send it: %s", hint)
	}
	if !strings.Contains(hint, "persistent") {
		t.Errorf("the refusal does not say WHY that agent is the durable one: %s", hint)
	}
}
...
// agent told to send its report to an agent that is not there loses it twice.
func TestNoOperatorMeansNoFallbackIsClaimed(t *testing.T) {
	...
	if strings.Contains(le2.Hint, "the operator is on this board") {
		t.Errorf("a fallback was offered on a board with no human agent: %s", serr)
	}
}
