...
// not.
//
// And not in the channel e2e either: an automatic join needs a board where a
// shared identifying ref is the strongest match, and in a suite with accumulated
// lanes the second agent matched something else entirely. A test that must win a
...
func TestTheMatcherCarriesTheCoordinationKeyOutOfAnAutoJoin(t *testing.T) {
	...
	opened, err := e.Do(ctx, &core.Op{
		Kind: core.OpLaneOpen, Token: owner, Channel: "auth-work", Text: "auth",
	})
	if err != nil {
		t.Fatalf("setup: lane_open: %v", err)
	}
	want, _ := opened["key"].(string)
	if want == "" {
		t.Fatal("setup: lane_open returned no key; this test is pinning the wrong field")
	}

	action, _, key := e.attemptJoin(ctx, joiner,
		core.LaneMatch{Lane: "auth-work", Score: 0.9},
