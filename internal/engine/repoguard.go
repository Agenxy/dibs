...
// declaring agent was actually in, even though the agent records its cwd.
//
...
// auto-joined to that repository's coordination agent on the strength of a
...
// are real and Dibs should not pretend to know better. What it will not do is
// join them automatically, which is the part that costs an agent its attention
// for nothing.
//
// # Not knowing is not evidence
//
// An agent with no cwd, a plain HTTP client, an older harness, is NOT gated. The
