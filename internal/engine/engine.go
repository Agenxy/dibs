...
type Engine struct {
	...
	resumeAt map[string]time.Time // per-lane resume rate limit (1/10s)
	watch    []waiter
	...
	// footprints backfills lanes opened before the index was ready. Ephemeral:
	...
	// human is the operator's own lane, so a person can join lanes and speak in
...
// boot applies SPEC §7's evidence rule: grace to boot+TTL only for lanes whose
...
func (e *Engine) exec(op *core.Op, now time.Time) (core.Result, error) {
	op.Lane = "" // replay-only actor field; never trusted from ingress
	...
	// System ops carry no lane token: they are generated by the engine itself,
	...
	// A system op carries no lane token, and one that does is refused.
	//
	...
	if !system {
		...
		case core.OpResumeLane:
			// Phase 3 for resumes: 1/10s per lane, keyed via nonce.
...
func (e *Engine) applyAndLedger(op *core.Op, now time.Time) (core.Result, error) {
	...
	//
	// That is not theoretical. This board reached a state where `close_lane`
	...
	if e.state.Serial != before {
		if lerr := e.led.Append(e.state.Serial, now, op); lerr != nil {
			panic(fmt.Sprintf("lanes: ledger persistence failure (fail-stop, SPEC §4): %v", lerr))
		}
		e.publish(evs)
	}
	return res, nil
...
// from a dormant/stale lane (SPEC §2). Must run inside the loop.
func (e *Engine) wakeIfSleeping(l *core.Lane, now time.Time) {
	if !l.Sleeping() {
		return
	}
	_, _ = e.applyAndLedger(&core.Op{Kind: core.OpWakeLane, Token: l.Token, Lane: ""}, now)
}
...
func (e *Engine) publish(evs []core.Event) {
	...
	for _, ev := range evs {
		// A lane that has gone must take its cached footprint with it, HERE, at
		// the moment it goes.
		//
	...
	for _, w := range e.watch {
		matched := filterEvents(evs, w.lane, w.all)
		if len(matched) > 0 {
			w.ch <- matched
		} else {
			keep = append(keep, w)
...
func (e *Engine) cancelWaiters(lane string) {
	var keep []waiter
	for _, w := range e.watch {
		if w.lane == lane {
...
func filterEvents(evs []core.Event, lane string, all bool) []core.Event {
	if all {
		return evs
	}
	var out []core.Event
	for _, ev := range evs {
		if ev.To == "" || ev.To == lane || ev.Lane == lane {
			out = append(out, ev)
		}
	}
	return out
...
func (e *Engine) eventsSince(serial uint64, lane string, all bool) []core.Event {
	i := 0
	for i < len(e.ring) && e.ring[i].Serial <= serial {
		i++
	}
	return filterEvents(e.ring[i:], lane, all)
}
...
// back in front of an agent before Lanes stops asking (SPEC-CHANNELS.md §7).
//
