...
func (e *Engine) exec(op *core.Op, now time.Time) (core.Result, error) {
	...
	//
	// host_renamed is the daemon rewriting its OWN rows onto the fleet
	// identity it adopted at startup (cmd/dibd, renameHost). It was left off
	// this list for a release: the submit is tokenless, so authentication
