...
const (
	// MatchOff — no repository configured. Matching is a feature the operator
	// has not turned on, which is a legitimate state and must say so.
	MatchOff MatchPhase = "off"
	// MatchIndexing — configured and still reading. Declarations made now
	// genuinely cannot be matched, and the agent should know to declare again.
	MatchIndexing MatchPhase = "indexing"
	// MatchReady — working. An empty result here means "no overlapping work",
	// which is real information.
	MatchReady MatchPhase = "ready"
	// MatchDegraded — an embedding service was configured and could not be
	...
	// MatchNoThreshold — scoring works but no join bar was calibrated, so lanes
