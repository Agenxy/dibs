...
// show_board promises one cheap summary line because agents call it repeatedly,
// but the complete board was also returned in structuredContent. Hosts that put
// that base-MCP field into model context charged the agent for every lane and
...
func TestShowBoardDefaultsToOneSummaryAndMakesDetailExplicit(t *testing.T) {
	...
	if !strings.HasPrefix(text, "Lanes board:") {
	...
	if strings.Contains(string(encoded), marker) {
		t.Fatal("default show_board still returns the full board outside its summary")
	}
	meta, _ := result["_meta"].(map[string]any)
	encoded, _ = json.Marshal(meta)
	if !strings.Contains(string(encoded), marker) {
		t.Fatal("default show_board withheld board detail from the human panel as well as the model")
	}

	detailed := rawToolResult(t, srv, "show_board", map[string]any{
...
// ack_board is the recovery checkpoint after context loss. Both content shapes
...
func TestAckBoardKeepsRecoveryKeysInEveryModelFacingShape(t *testing.T) {
	srv, _ := newServer(t)
	registered := toolCall(t, srv, "register_lane", map[string]any{"name": "recovering"})
	token := registered["token"].(string)

	result := rawToolResult(t, srv, "ack_board", map[string]any{"token": token})
	content := result["content"].([]any)[0].(map[string]any)["text"].(string)
	var plain map[string]any
	if err := json.Unmarshal([]byte(content), &plain); err != nil {
		t.Fatalf("ack_board content is not JSON: %v", err)
	}
...
// fact: who to coordinate with. lane_read returned members:2 even though the
...
// or acknowledge anything merely because the lane was read.
func TestLaneReadNamesItsMembers(t *testing.T) {
	srv, _ := newServer(t)
	alpha := toolCall(t, srv, "register_lane", map[string]any{"name": "alpha"})
	beta := toolCall(t, srv, "register_lane", map[string]any{"name": "beta"})
	...
	toolCall(t, srv, "lane_open", map[string]any{
		"token": ta, "lane": "shared-work", "topic": "coordinate the shared work",
	})
	toolCall(t, srv, "lane_join", map[string]any{"token": tb, "lane": "shared-work"})

	read := toolCall(t, srv, "lane_read", map[string]any{"token": ta, "lane": "shared-work"})
	rawNames, ok := read["member_names"].([]any)
	if !ok {
		t.Fatalf("lane_read member_names = %T, want an array of lane names; result: %v",
	...
	if !slices.Equal(names, []string{"alpha", "beta"}) {
		t.Fatalf("lane_read member_names = %v, want alpha and beta", names)
	}
	if read["members"] != float64(2) {
		t.Fatalf("lane_read members = %v, want the compatible count 2", read["members"])
	}
}
...
func assertEmptyCheckpointLists(t *testing.T, carrier string, result map[string]any) {
	t.Helper()
	for _, key := range []string{"announcements", "lane_updates"} {
		...
		if len(items) != 0 {
			t.Errorf("%s %s = %v, want empty on a fresh lane", carrier, key, items)
		}
	}
}
...
func TestACheckpointIsNeverReplacedByASmallerShape(t *testing.T) {
	srv, _ := newServer(t)
	registered := toolCall(t, srv, "register_lane", map[string]any{"name": "checkpointing"})
	token := registered["token"].(string)

	for _, tool := range []string{"ack_board", "inbox"} {
	...
	// ack_board specifically must still answer everything it owes.
	result := rawToolResult(t, srv, "ack_board", map[string]any{"token": token})
	text := result["content"].([]any)[0].(map[string]any)["text"].(string)
	var plain map[string]any
	if err := json.Unmarshal([]byte(text), &plain); err != nil {
...
func TestARelativeClaimPathIsRefusedRatherThanGuessedAt(t *testing.T) {
	srv, _ := newServer(t)
	registered := toolCall(t, srv, "register_lane", map[string]any{"name": "claimer"})
	token := registered["token"].(string)
	...
	ok := toolCall(t, srv, "claim", map[string]any{
		"token": token, "path": "/tmp/lanes-claim-probe", "mode": "shared",
	})
	if ok["granted"] != true {
