...
│ agent A     │──────────────────────────────────────────────▶  │  lanesd (daemon) │
│ agent B     │──────────────────────────────────────────────▶  │  single thread   │
│ agent C     │──────────────────────────────────────────────▶  │  in-memory state │
└─────────────┘                                                 └───────┬──────────┘
      ▲                                                                 │ append + fsync
      │ lanes CLI / web board (human view, decrypted)                   ▼
      └──────────────────────────────────────────────────  ~/.lanes/ledger.jsonl
```

- **`lanesd`**: single-threaded event loop; all mutations and reads execute sequentially
...
```json
{"s":1042, "t":"2026-07-22T18:04:11.302Z", "n":"a1b2c3d4", "e":"send_message", "prev":"9f2c…", "op":{…}}
```
...
- **Hash chain**: `prev` = SHA-256 of the previous line's raw bytes (covers ciphertext;
  `lanes verify` needs no key). Snapshots (v1.1) anchor the head hash; `lanes
...
- **Crash semantics (normative)**:
  ...
  - Order: apply in memory → encrypt/serialize/append/fsync → reply. **Any failure in
    ...
    lanesd exits rather than let memory diverge from disk.
  - **Indeterminate-commit window**: a crash after fsync but before the reply means a
    ...
    `send_message` takes a client `op_id`; `register_lane`/`resume_lane` are keyed by
...
- **Encryption at rest**: message bodies, responses, and lane tokens sealed with
...
- **Rotation. NOT IMPLEMENTED. Planned for v1.1.** The design is: at 64 MB / 1M
  ...
  corrupts and `lanes verify` stays correct; it simply gets slower and larger
  forever. A long-lived board wanting a fresh start archives the directory and
  starts a new one.

Ledgered op kinds: `register_lane, resume_lane, wake_lane, activity_checkpoint,
ack_board, update_lane, close_lane, heartbeat` (recovery only), `set_slot,
clear_slot, send_message, respond, ack_message, claim` (incl. renewals), `release,
...
| `harness` | `LANES_HARNESS` | any harness whose SDK will not say |
...
- **`session_id` was never set, so reattach never fired.** Reattach keys on
  ...
  new bridge and correctly gets a new lane. Verified both ways.

  The id is `bridge-<pid>-<random>`, not bare PID: PIDs are recycled, and a
  recycled PID would silently reattach a fresh agent onto a dead agent's lane
  and its mail.
...
- **Lane token**: 256-bit CSPRNG hex, returned by `register_lane`/`resume_lane`.
  Write + mailbox-read capability for one lane. Passed as a **tool argument**
...
- **Registration nonce**: client-generated, ≥128-bit CSPRNG, **required for
  ...
  - *Response-loss retry*: `register_lane` with a nonce it has seen, while the lane is
    active and was created within one lane TTL, returns the original result
...
- v2: Ed25519 per-lane signatures; UDS peer credentials as an alternative transport
  gate.
...
*received*, probes, and other lanes' ops never touch it: another agent cannot keep
an abandoned lane looking active.
...
**v1 is store-and-catch-up, not wake-on-mail**: mail to a dormant lane waits for the
...
- **Restart grace, cumulatively bounded**: at boot, a lane gets grace to
  ...
  it. Incoming mail and other lanes' activity are irrelevant by construction (§6).
...
Messages go lane → lane; identity = send serial; bodies private (§4, §5).

| Type | Expects | Dispositions (recipient) |
|---|---|---|
| `notify` | nothing | optional `ack_message` |
| `question` | an answer | `answer`, `decline` |
| `request` | a decision | `approve`, `deny`, `decline` |
| `handoff` | nothing | optional `ack_message` |
...
| Requested \ Existing (another lane's) | none | `shared` | `exclusive` |
|---|---|---|---|
| `shared` | ✅ | ✅ | ❌ |
| `exclusive` | ✅ | ❌ | ❌ |
...
- **Cursor recovery, one atomic checkpoint**: the event ring holds the most recent
  ...
  pending mail are effects of this same `ack_board` op, at its single serial; the
...
|---|---|---|
| ops per lane | 10/s, burst 30 | `E_RATE_LIMITED` (no wake, no ledger) |
| live lanes / persistent lanes | 64 / 16 | `E_LANE_LIMIT` |
| slots per lane | 32 | `E_SLOT_LIMIT` |
| claims per lane / global | 32 / 256 | `E_CLAIM_LIMIT` |
| mailbox depth (non-terminal) | 256 | §8 backpressure |
| terminal messages retained | 128 per lane, then GC'd (ledger keeps history) | pruned oldest-first |
| archived lanes retained in state | 7 days, then GC'd (with nonces + dedup records) | pruned |
...
| resume_lane rate | 1 per 10 s per lane | `E_RATE_LIMITED` |
| lane lease TTL | 5 min | → `stale`/`dormant` |
...
| deadline | 10 min default; 2 h max (7 d to persistent lanes) | §7 cascade |
...
`bind_session`, `broadcast`, `all_mail`, `show_board`), and v1.2 added the
...
| Tool | Purpose |
|---|---|
| `register_lane(name, description?, pid?, nonce?, kind?)` | → `{lane_id, token, serial, board}`; nonce required for `kind: persistent` |
| `resume_lane(nonce, resume_id, pid?)` | reactivate a persistent lane: rotates token, bumps activation generation, rebinds PID, wakes, re-arms gate; idempotent per resume_id (§5) |
| `ack_board()` | pass the awareness gate (per activation); → atomic `{board, inbox, serial}` checkpoint (§10) |
| `update_lane(description)` / `close_lane()` | lifecycle |
| `heartbeat()` | renew lease while idle (implicit on every call) |
| `set_slot(slot_id?, text, dirs?)` / `clear_slot(slot_id)` | declare/end work units |
| `send_message(to, type, body, deadline_s?, op_id?)` | → `msg_serial`; `op_id` = durable dedup (§4) |
| `respond(msg_serial, disposition, body?)` | answer/approve/deny/decline |
| `ack_message(msg_serial)` | explicit read receipt |
| `inbox()` | non-terminal + unconsumed terminal mail, decrypted; marks delivery; returns `truncated_before_serial` (§8) |
| `get_message(msg_serial)` | full message + response; sender or recipient (§8) |
...
**Resources**: `lanes://board`. Mailboxes are deliberately not resources.

## 13. Human window

`lanes board` · `lanes messages` · `lanes log [--follow]` · `lanes verify [path]`
(keyless) · `lanes mcp-config` (prints host config incl. local-secret header) ·
`lanes version`. Web board at `/`: server-rendered, SSE-live (per-op frames, §10),
...
Standards win wherever they cover a concept; Lanes invents only the uncovered core
...
Overrides live in `<dir>/lanes.toml` (`addr`, `tls_cert`, `tls_key`, `insecure_plaintext`)
for operators who want their own CA or a fronting proxy. Sovereignty is the rule: Lanes
...
ledgered wake transitions; ephemeral + persistent lanes; resume_lane; awareness gate
per activation; mailbox (full state machine, get_message, op_id dedup,
...
SSE web board; static binaries (`lanesd` + `lanes`, no cgo, no runtime deps).

**v1.1**: rotation + snapshots; kqueue/pidfd exit notification; `lanes watch --exec`
(wake-on-mail supervisor glue); `lanes limits`; `lanes audit`; fuzz + crash
