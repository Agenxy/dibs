...
func registerScorerFlags() *scorerFlags {
	...
	flag.StringVar(&f.repo, "match-repo", "",
		"repository to mine for work-overlap matching (enables lane auto-matching; run `lanes calibrate` first)")
	flag.Float64Var(&f.join, "match-join", 0,
		"auto-join lanes at or above this overlap score (0 = suggest only, never join)")
	flag.Float64Var(&f.notify, "match-notify", 0,
		"mention lanes at or above this overlap score")
	...
	flag.BoolVar(&f.director, "match-director-required", false,
		"gate every matched join on a coordinator admitting it (lane_admit). "+
...
func (f *scorerFlags) applyConfig(c MatchConfig) {
	...
	if c.Deadline != "" {
		if d, err := time.ParseDuration(c.Deadline); err == nil {
			...
			slog.Warn("ignoring unparseable match.deadline in lanes.toml",
...
func (f *scorerFlags) applyEnv() {
	...
	str("repo", "LANES_MATCH_REPO", &f.repo)
	num("join", "LANES_MATCH_JOIN", &f.join)
	num("notify", "LANES_MATCH_NOTIFY", &f.notify)
	str("embed-url", "LANES_MATCH_EMBED_URL", &f.embedURL)
	str("embed-model", "LANES_MATCH_EMBED_MODEL", &f.embedModel)
	str("embed-key", "LANES_MATCH_EMBED_KEY", &f.embedKey)
}
...
func (f *scorerFlags) install(ctx context.Context, eng *engine.Engine) {
	...
	if repo == "" {
		...
		slog.Info("work-overlap matching is off",
			"enable", "-match-repo <path>, or [match] repo = \"…\" in lanes.toml")
		return
	}
	...
	if em, ok := scorer.(*overlap.Embed); ok && !em.Recognised() {
		...
		if q == "" && d == "" {
			slog.Warn("embedding model is being addressed symmetrically",
				"model", f.embedModel,
				"why", "its name matches no retrieval convention Lanes knows",
				...
				"fix", "pass -match-embed-query-prefix (and -match-embed-doc-prefix if the "+
					"family uses one), or run `lanes calibrate` to measure the difference")
		}
	}
	if f.join == 0 {
...
func (f *scorerFlags) withSidecar(ctx context.Context, base overlap.Scorer) overlap.Scorer {
	...
	// An explicit convention beats an inferred one: a model family Lanes has
