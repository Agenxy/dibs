...
// not chosen: `lanes calibrate` prints thresholds for a specific repository,
...
func TestUnknownConfigKeysAreRefusedNotIgnored(t *testing.T) {
	for _, tc := range []struct{ name, body, want string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			dir := t.TempDir()
			if err := os.WriteFile(filepath.Join(dir, "lanes.toml"), []byte(tc.body), 0o600); err != nil {
				t.Fatal(err)
			}
	...
	if err := os.WriteFile(filepath.Join(dir, "lanes.toml"), []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
	c, err := loadConfig(dir)
	if err != nil {
...
`
	...
	dir2 := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir2, "lanes.toml"),
		[]byte("[match]\nembed_key = \"sk-secret\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestPrecedenceIsFlagThenEnvThenFile_AndZeroIsAValue(t *testing.T) {
	...
	t.Run("env beats file", func(t *testing.T) {
		t.Setenv("LANES_MATCH_REPO", "/from/env")
		t.Setenv("LANES_MATCH_JOIN", "0.9")
		f := defaultScorerFlags()
		f.applyConfig(file)
		if f.repo != "/from/env" {
	...
	t.Run("an explicit env zero beats a nonzero file", func(t *testing.T) {
		t.Setenv("LANES_MATCH_JOIN", "0")
		f := defaultScorerFlags()
		f.applyConfig(file)
		if f.join != 0 {
			t.Errorf("LANES_MATCH_JOIN=0 means 'never auto-join' and must win, got %v", f.join)
		}
	})

	t.Run("an explicit flag zero beats a nonzero env", func(t *testing.T) {
		t.Setenv("LANES_MATCH_JOIN", "0.5")
		f := defaultScorerFlags()
...
// The daemon must honour LANES_ADDR, and must not claim to be up before it is.
//
// Both were found by installing from a cold clone and following the README.
// LANES_ADDR is read by every other Lanes binary; lanesd ignored it and bound
...
func TestTheDaemonAnnouncesItselfOnlyAfterItHasThePort(t *testing.T) {
	...
	if !strings.Contains(text, `os.Getenv("LANES_ADDR")`) {
		t.Error("lanesd does not read LANES_ADDR, which every other Lanes binary honours")
	}
	...
	if announce < 0 {
		t.Fatal(`no "lanesd up" line to order`)
	}
	if bind > announce {
