...
func TestDoctorDoesNotReadTheLocalWakeConfigForARemoteBoard(t *testing.T) {
	...
	var what, fix string
	checkWakeRoutes(dir, &boardView{Node: "hub-9"}, hubHosts{},
	...
	called := false
	checkWakeRoutes(dir, &boardView{Node: "local-1"}, hubHosts{}, func(string) { called = true }, func(string, string) { called = true })
	if !called {
...
func TestDoctorSaysWhenBothWakeRoutesAreOff(t *testing.T) {
	...
	var what, fix string
	checkWakeRoutes(dir, &boardView{Node: "local-1"}, hubHosts{},
