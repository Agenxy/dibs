...
func TestBoardJSONCarriesWhatTheProseBoardShows(t *testing.T) {
	daemon := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `{
			...
			"claims": [{"agent": "codex-1", "path": "/tmp/x", "mode": "exclusive",
				"note": "migration", "renewed": "2026-08-11T10:00:00Z"}],
			"spaces": [{"id": "reconnects", "topic": "session reconnects",
	...
	if len(doc.Agents) != 1 || doc.Agents[0].ID != "codex-1" ||
		doc.Agents[0].Status != "stale" || doc.Agents[0].StaleReason != "process_exited" ||
		doc.Agents[0].LastSeen == "" {
		t.Errorf("the agent facts the prose board shows are missing: %+v", doc.Agents)
	}
	if len(doc.Agents[0].Slots) != 1 || doc.Agents[0].Slots[0].Text != "fixing reconnects" {
		t.Errorf("declared work is missing: %+v", doc.Agents[0].Slots)
	}
	if len(doc.Claims) != 1 || doc.Claims[0].Mode != "exclusive" || doc.Claims[0].Path != "/tmp/x" {
		t.Errorf("claims are missing: %+v", doc.Claims)
	}
	if len(doc.Spaces) != 1 || doc.Spaces[0].Unacked != 2 {
		t.Errorf("space counts are missing: %+v", doc.Spaces)
	}
}

// An empty board is a normal state a script iterates over, so the lists have
// to be lists: null decodes fine in Go and trips over the first `for agent in
// doc["agents"]` anywhere else.
func TestBoardJSONPinsEmptyListsToEmptyLists(t *testing.T) {
	daemon := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = fmt.Fprint(w, `{"serial": 0, "node": "n1", "agents": []}`)
	}))
	defer daemon.Close()
	t.Setenv("DIBS_DIR", t.TempDir())
	t.Setenv("DIBS_ADDR", strings.TrimPrefix(daemon.URL, "http://"))
	...
	for _, key := range []string{`"agents":[]`, `"claims":[]`, `"spaces":[]`} {
...
// writeLedger puts a small parseable ledger where `dibs log` will look.
func writeLedger(t *testing.T, records int) {
	t.Helper()
	dir := t.TempDir()
	var b strings.Builder
	for i := 1; i <= records; i++ {
		fmt.Fprintf(&b, `{"s":%d,"t":"2026-08-11T0%d:00:00Z","e":"register","op":{"agent":"codex-%d","to":""}}`+"\n",
			i, i, i)
	}
...
// the prose columns: serial, time, op, agent. The names are the document's
...
func TestLogJSONEmitsOneParseableObjectPerEvent(t *testing.T) {
	...
	for i, line := range lines {
		...
		if rec.Serial != uint64(i+1) || rec.Op != "register" ||
			rec.Agent != fmt.Sprintf("codex-%d", i+1) || rec.Time == "" {
