...
// disagree.
//
// `dibs version` used to print one line about itself, which is the least useful
// half of the answer. `dibs` and `dibd` are separate processes: installing a
...
func printVersion() {
	...
	if err != nil {
		...
		// that says less.
		fmt.Println("dibd  not running (nothing listening on " + addr() + ")")
		return
	}
	...
	if info.StartedAt == "" {
		...
		fmt.Printf("dibd  %s  (running a build older than this CLI: it does not\n", info.Version)
		...
		fmt.Println()
		fmt.Println("      dibs stop && dibd &")
		return
	}
	fmt.Printf("dibd  %s  started %s\n", info.Version, humanAge(info.StartedAt))
	if info.PanelBuild != "" {
