...
// dibd` had and the reason `dibs stop` exists.
...
// `dibs stop --help` stopped the daemon and exited 0: asking a destructive
// command what it does performed it. Second time in this CLI: configure.go
// carries a comment about `dibs configure --help` being read as a directory
...
// and threw os.Args[2:] away, so `dibs stop --help` signalled the daemon. A
...
func TestTheStopVerbIsWiredThroughArgumentChecking(t *testing.T) {
	...
	if !strings.Contains(body, "stop(os.Args[2:])") {
		t.Errorf("`dibs stop` does not route through the argument check.\n"+
			"  Calling stopDaemon directly means flags are ignored, and `dibs stop --help`\n"+
			"  performs the stop. Dispatch found:\n%s", body)
	}
}

// A command's verdict on an argument must not depend on where the argument sits.
// `dibs stop --help --force` printed help and exited 0, silently accepting a
...
func TestUnknownArgumentsAreRefusedWhicheverSideOfHelpTheySit(t *testing.T) {
	for _, args := range [][]string{
		...
		if err := stop(args); err == nil {
			t.Errorf("dibs stop %v was accepted; an unsupported argument means the "+
				"caller expects behaviour this command does not have", args)
		}
		if err := serviceCommand(args); err == nil {
			t.Errorf("dibs configure --service %v was accepted", args)
		}
	}
