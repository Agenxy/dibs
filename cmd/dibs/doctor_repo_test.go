...
// project's paths as evidence of shared work with no way, from any Dibs
...
// is pinned. /Users/x/Dibs-old must not read as inside /Users/x/Dibs.
func TestUnderDirDoesNotConfuseSiblingsForChildren(t *testing.T) {
	base := t.TempDir()
	repo := filepath.Join(base, "Dibs")
	for _, tc := range []struct {
		...
		{
			filepath.Join(base, "Dibs-old"), false,
