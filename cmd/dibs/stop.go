...
//
// `dibs stop --help` stopped the daemon and exited 0. The dispatch ignored
...
// about `dibs configure --help` being taken as a directory named "--help".
...
func stop(args []string) error {
	...
	// the verdict depend on argument ORDER: `dibs stop --help --force` printed
...
//
// The documentation used to say `pkill dibd`, which is wrong in a product
...
func stopDaemon(dir string) error {
	...
	for time.Now().Before(deadline) {
		...
		if gone := proc.Signal(syscall.Signal(0)) != nil; gone {
			fmt.Printf("stopped dibd (pid %d) on %s\n", d.PID, dir)
			return nil
		}
