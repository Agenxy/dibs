...
func TestStampIsEmittedOnlyForTheOneCaseThatDeserevesIt(t *testing.T) {
	lane := func(string, string) string { return "builder" }
	...
	got := stampFor(strings.NewReader(hook("codex exec 'review'")), lane)
	if !strings.Contains(got, `LANES_PARENT=builder codex exec`) {
		t.Errorf("a real agent spawn from a known lane was not stamped: %q", got)
	}
