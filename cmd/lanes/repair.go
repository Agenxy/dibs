...
func repairLedger(args []string) error {
	...
	for _, a := range args {
		if a == "--yes" || a == "-y" {
			...
			return fmt.Errorf("`lanes admin repair-ledger` takes only --yes, got %q", a)
		}
	}
	path := ledgerPath()
	...
	fmt.Printf("  start the daemon again: lanesd\n")
	return nil
}
...
func lastReplayableSerial(dir string) (good, total int, firstBad uint64, applyErr error, err error) {
	...
	// #nosec G304 -- the board's own data directory, named by -dir/LANES_DIR.
