...
func TestEveryDispatchedVerbHasAManPageEntry(t *testing.T) {
	...
	handled := regexp.MustCompile(`(?m)^\tcase "([a-z-]+)"`).FindAllStringSubmatch(string(src), -1)
	if len(handled) < 10 {
	...
	for _, m := range handled {
		if !strings.Contains(page, ".It Cm "+m[1]) {
			t.Errorf("`lanes %s` is dispatched but has no entry on the man page:\n"+
				"  a man page that skips a verb documents a different program", m[1])
		}
	}
}
...
func TestEveryEnvironmentVariableIsOnTheManPage(t *testing.T) {
	...
	vars := regexp.MustCompile(`LANES_[A-Z_]+`).FindAllString(usage, -1)
	if len(vars) < 3 {
		t.Fatalf("found only %d LANES_* variables in the usage text: the env line "+
...
func TestManPageLintsCleanUnderMandoc(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "lanes.1")
