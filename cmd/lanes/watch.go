...
func watch(args []string) error {
	...
	fmt.Fprintf(os.Stderr, "lanes watch: running `%s` on each new message (^C to stop)\n", *execCmd)

	for {
		select {
		case <-stop:
			fmt.Fprintln(os.Stderr, "lanes watch: stopped")
			return nil
		default:
		}
		...
		for _, raw := range evs {
			...
			// this lane, so every message.sent here is for us.
...
// runExec runs the command with the event on stdin and LANES_* env vars set.
func runExec(command string, ev, data map[string]any) {
	...
	mt, _ := data["msg_type"].(string)
	fmt.Fprintf(os.Stderr, "lanes watch: fired on serial %s (%s from %s)\n", serial, mt, from)

	// #nosec G204 -- running the operator's own command IS the feature: `lanes watch
	...
	c.Env = append(
		os.Environ(),
		"LANES_EVENT="+string(evJSON),
		"LANES_EVENT_TYPE=message.sent",
		"LANES_EVENT_SERIAL="+serial,
		"LANES_MSG_TYPE="+mt,
		"LANES_MSG_FROM="+from,
	)
	...
	if err := c.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "lanes watch: exec error: %v\n", err)
	}
}
