...
import (
	...
	"github.com/agenxy/lanes/internal/adminpw"
...
// adminCmd handles `lanes admin <sub>`.
func adminCmd(args []string) error {
	...
	case "prune":
		...
		return adminOnly("admin prune", func() error { return pruneLanes(lane) })
	case "repair-ledger":
	...
	case "coordinator", "member", "admin":
		if len(args) < 2 {
			...
			return fmt.Errorf("`lanes admin %s` needs a lane name: lanes admin %s <lane>\n"+
				"  `lanes board` lists the lanes on this board", args[0], args[0])
		}
		role, lane := args[0], args[1]
		return adminOnly("admin "+role, func() error { return setLaneRole(lane, role) })
	default:
		// A mistyped verb granted nothing and exited 0, which reads as done.
		// Bare `lanes admin` is a genuine request for the list and still prints
...
const adminUsage = `usage:
  lanes admin set-password         set the password gating the board and decrypted mail
  lanes admin repair-ledger        recover a board whose ledger will not replay
  lanes admin coordinator <lane>   promote: may broadcast and force-release claims
  lanes admin admin <lane>         promote: everything you can do, INCLUDING reading all mail
  lanes admin member <lane>        demote back to a plain member
  lanes admin prune [lane]         close finished lanes; no argument clears every
                                   lane that is not live (a crashed lane cannot
...
func pruneLanes(lane string) error {
	pass, err := promptAdminForGodView()
	if err != nil {
		return err
	}
	body, _ := json.Marshal(map[string]string{"lane": lane})
	...
	req.Header.Set("X-Lanes-Admin", pass)
...
func setLaneRole(lane, role string) error {
	pass, err := promptAdminForGodView()
	if err != nil {
		return err
	}
	body, _ := json.Marshal(map[string]string{"lane": lane, "role": role})
...
func setAdminPassword() error {
	...
	p1, err := readPassword("Set Lanes admin password (gates the board / decrypted mail): ")
	if err != nil {
		return err
	}
	if len(p1) < 8 {
