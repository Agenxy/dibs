...
func calibrate(args []string) error {
	...
	if cal.Measured() {
		fmt.Fprintln(os.Stderr, "\nnothing was written; set these in your Lanes config if they look right")
	}
	return nil
}
...
func buildEmbedScorer(ctx context.Context, dir, url, model, key, qPrefix, dPrefix string) (*overlap.Embed, error) {
	...
	// An explicit convention beats an inferred one: a family Lanes has never
...
func printThresholds(cal overlap.Calibration) {
	...
	case !cal.Measured():
		...
		fmt.Printf("  is nothing to take a percentile of. The numbers below are Lanes' built-in\n")
...
func warnIfZeroJoin(cal overlap.Calibration) {
	...
	fmt.Fprintln(os.Stderr,
		"NOTE: a join_threshold of 0 does not mean 'join on any overlap'. 0 is the\n"+
			...
			"lanes look relevant; it just will not move them automatically.")
}
