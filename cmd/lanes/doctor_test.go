...
http_headers = { "X-Lanes-Local" = "` + strings.Repeat("a", 64) + `" }
`
...
// before the Lanes one. Anchoring on the secret is not enough either: a config
...
`
	...
	if len(got) != 1 || got[0] != "http://127.0.0.1:4777/mcp" {
		t.Fatalf("only the Lanes endpoint belongs in the hint, got %v", got)
	}
}
...
func TestAConfigWithNoURLIsAssumedToBeForThisDaemon(t *testing.T) {
	const stdio = `{"mcpServers":{"lanes":{"command":"lanes","args":["mcp-stdio"]}}}`
