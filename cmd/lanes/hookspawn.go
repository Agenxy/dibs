...
// hookSpawn stamps a spawned subagent with the lane that spawned it.
//
// # Why this exists
//
...
func stampFor(r io.Reader, resolve func(sessionID, cwd string) string) string {
	...
	if strings.Contains(cmd, "LANES_PARENT=") {
		return "" // already stamped, by an outer hook or by the agent itself
	}
	if !spawnsAgent(cmd) || !safeToPrefix(cmd) {
		return ""
	}
	lane := resolve(in.SessionID, in.CWD)
	if lane == "" {
		return ""
	}
	input["command"] = "LANES_PARENT=" + lane + " " + cmd
...
// laneForSession asks the daemon which lane owns a harness session.
//
// Via hook_poll, which is the existing token-less lifecycle-hook path: a hook
// has no lane token, and inventing a second unauthenticated endpoint for this
