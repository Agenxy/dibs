...
// would never reach register_lane.
...
func enrichRegister(line []byte) []byte {
	...
	params, _ := msg["params"].(map[string]any)
	if params == nil || params["name"] != "register_lane" {
		return line
	}
	if lastClientInfo != nil {
	...
	// LANES_HARNESS names the harness when its MCP client will not.
	//
	...
	// genuinely new session gets a new bridge and correctly gets a new lane.
